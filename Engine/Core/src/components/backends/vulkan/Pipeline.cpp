...
namespace vex {
    ...
    void VulkanPipeline::createTransparentPipeline(
        const std::string& vertShaderPath,
        const std::string& fragShaderPath,
        const VkVertexInputBindingDescription& bindingDescription,
        const std::vector<VkVertexInputAttributeDescription>& attributeDescriptions
        ...
        void VulkanPipeline::createMaskedPipeline(
            const std::string& vertShaderPath,
            const std::string& fragShaderPath,
            const VkVertexInputBindingDescription& bindingDescription,
            const std::vector<VkVertexInputAttributeDescription>& attributeDescriptions
        ...
        const std::vector<VkVertexInputAttributeDescription>& attrDesc)
    {
        ...
        auto createShaderModule = [&](const std::vector<char>& code) {
            ...
            VkShaderModule shaderModule;
            if (vkCreateShaderModule(m_r_context.device, &createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
                throw_error("Failed to create UI shader module");
            }
            return shaderModule;
        };
        ...
        if (vkCreatePipelineLayout(m_r_context.device, &pipelineLayoutInfo,
                                   nullptr, &m_layout) != VK_SUCCESS) {
            throw_error("Failed to create UI pipeline layout!");
        }
        ...
        if (vkCreateGraphicsPipelines(m_r_context.device, VK_NULL_HANDLE, 1,
                                      &pipelineInfo, nullptr, &m_pipeline) != VK_SUCCESS) {
            throw_error("Failed to create UI graphics pipeline");
        }
    ...
    void VulkanPipeline::createFullscreenPipeline(const std::string& vertPath, const std::string& fragPath) {
        ...
        if (vkCreatePipelineLayout(m_r_context.device, &pipelineLayoutInfo, nullptr, &m_layout) != VK_SUCCESS) {
            throw_error("Failed to create fullscreen layout");
        }
        ...
        if (vkCreateGraphicsPipelines(m_r_context.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_pipeline) != VK_SUCCESS) {
            throw_error("Failed to create fullscreen pipeline");
        }

        vkDestroyShaderModule(m_r_context.device, vertModule, nullptr);
        vkDestroyShaderModule(m_r_context.device, fragModule, nullptr);
    }

    #if DEBUG
        void VulkanPipeline::createDebugPipeline(const std::string& vertShaderPath, const std::string& fragShaderPath) {
            log("Creating Debug Pipeline...");
            ...
            auto createShaderModule = [&](const std::vector<char>& code) {
                ...
                VkShaderModule shaderModule;
                if (vkCreateShaderModule(m_r_context.device, &createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
                    throw_error("Failed to create debug shader module");
                }
                return shaderModule;
            };
            ...
            attrDesc[1].offset = sizeof(float) * 3;
            ...
            pipelineLayoutInfo.pSetLayouts = &m_r_context.uboDescriptorSetLayout;

            if (vkCreatePipelineLayout(m_r_context.device, &pipelineLayoutInfo, nullptr, &m_layout) != VK_SUCCESS) {
                throw_error("Failed to create debug pipeline layout");
            }
            ...
            if (vkCreateGraphicsPipelines(m_r_context.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_pipeline) != VK_SUCCESS) {
                throw_error("Failed to create debug graphics pipeline");
            }
